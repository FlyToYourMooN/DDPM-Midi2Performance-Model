...
def process_data(config, pianoroll_list, wav_list):
    ...
    for i in tqdm(range(num_musics)):
        ...
        pianoroll = process_pianoroll(pianoroll_list[i], len(mel[0]), config.data.hop_length)
...
def Main(config):
    ...
    dataset = np.load(open('/disk2/MusicNet/musicnet.npz','rb'), encoding = 'latin1', allow_pickle=True)
