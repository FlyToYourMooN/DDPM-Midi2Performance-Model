...
class DPM_Solver:
    ...
    def dpm_solver_second_update(
        self, x, s, t, y, cond, r1=0.5, noise_s=None, return_noise=False
    ):
        """
        ...
        if adaptive_step_size:
            with torch.no_grad():
                x = self.dpm_solver_adaptive(
                    x, order=order, t_T=t_T, t_0=t_0, atol=atol, rtol=rtol
                )
        else:
            if fast_version:
                orders, timesteps = self.get_time_steps_for_dpm_solver_fast(
                    t_T=t_T, t_0=t_0, steps=steps, device=device
                )
            else:
                N_steps = steps // order
                orders = [
