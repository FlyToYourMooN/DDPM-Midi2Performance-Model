...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1., time_input_type='1', total_N=1000, model_kwargs={}):
    ...
    def model_fn(x, t_continuous,y,cond):
        """
        ...
        if is_cond_classifier:
            ...
            t_discrete = get_model_input_time(t_continuous)
            return model(x,t_discrete,y=y,cond=cond,**model_kwargs)
...
class DPM_Solver:
    ...
    def dpm_solver_first_update(self, x, s, t,y,cond, return_noise=False):
        """
        ...
        noise_s = self.model_fn(x, s,y,cond)
        x_t = (
    ...
    def dpm_solver_second_update(self, x, s, t,y,cond, r1=0.5, noise_s=None, return_noise=False):
        """
        ...
        noise_s1 = self.model_fn(x_s1, s1,y,cond)
        x_t = (
            torch.exp(log_alpha_t - log_alpha_s)[(...,) + (None,)*dims] * x
            - (sigma_t * phi_1)[(...,) + (None,)*dims] * noise_s
    ...
    def dpm_solver_third_update(self, x, s, t,y,cond, r1=1./3., r2=2./3., noise_s=None, noise_s1=None, noise_s2=None):
        """
        ...
        if noise_s2 is None:
            ...
            noise_s2 = self.model_fn(x_s2, s2,y,cond)
        x_t = (
            torch.exp(log_alpha_t - log_alpha_s)[(...,) + (None,)*dims] * x
            - (sigma_t * phi_1)[(...,) + (None,)*dims] * noise_s
    ...
    def dpm_solver_update(self, x, s, t, order,y,cond):
        """
        ...
        """
        if order == 1:
            return self.dpm_solver_first_update(x, s, t,y,cond)
        elif order == 2:
            return self.dpm_solver_second_update(x, s, t,y,cond)
        elif order == 3:
            return self.dpm_solver_third_update(x, s, t,y,cond)
        else:
    ...
    def sample(self, x, steps=10, eps=1e-4, T=None, order=3, skip_type='logSNR',
        adaptive_step_size=False, fast_version=True, atol=0.0078, rtol=0.05,y=None,cond=None):
        """
        ...
        if adaptive_step_size:
            ...
            with torch.no_grad():
                for i, order in enumerate(orders):
                    ...
                    x = self.dpm_solver_update(x, vec_s, vec_t, order,y,cond)
        return x
